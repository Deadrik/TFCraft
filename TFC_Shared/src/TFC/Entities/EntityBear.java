...
public class EntityBear extends EntityTameableTFC
{
    ...
    public EntityBear (World par1World, float father_size, float mother_size)
    {
        super (par1World);
        texture = "/mob/Bear.png";
        setSize (1.2F, 1.2F);
        moveSpeed = 0.2F;
        size_mod += (mother_size + father_size - 1.9f);
        getNavigator ().setAvoidsWater (true);
        tasks.addTask (1, new EntityAISwimming (this));
        tasks.addTask (4, new EntityAIAttackOnCollide (this, moveSpeed * 1.5F, true));
        if (sex == 0)
    ...
    {
        EntityBear entitybear = new EntityBear (worldObj, size_mod, ((EntityAnimalTFC)par1EntityAnimal).size_mod);
        return entitybear;
    }
