...
public class WorldGenCaveDecor implements IWorldGenerator
{
	...
	{
		int id = world.getBlockId(x, y, z);
		if(!world.isRemote && y > 8 && id == 0 && TFC_Core.isRawStone(world, x, y+1, z))
		{
			if(world.getBlockId(x, y-1, z) == 0 && world.getBlockId(x, y-2, z) == 0 && world.getBlockId(x, y-3, z) == 0)
			{

				world.setBlock(x, y, z, TFCBlocks.stoneStalac.blockID, 0, 0x2);
				world.setBlock(x, y-1, z, TFCBlocks.stoneStalac.blockID, 0, 0x2);
				world.setBlock(x, y-2, z, TFCBlocks.stoneStalac.blockID, 0, 0x2);
				return true;
			}
		}
		return false;
	...
	{
		int id = world.getBlockId(x, y, z);
		if(!world.isRemote && y <128 && id == 0 && TFC_Core.isRawStone(world, x, y-1, z) && !world.canBlockSeeTheSky(x, y, z))
		{
			if(world.getBlockId(x, y+1, z) == 0 && world.getBlockId(x, y+2, z) == 0 && world.getBlockId(x, y+3, z) == 0)
			{
				world.setBlock(x, y, z, TFCBlocks.stoneStalac.blockID, 8, 0x2);
				world.setBlock(x, y+1, z, TFCBlocks.stoneStalac.blockID, 8, 0x2);
				world.setBlock(x, y+2, z, TFCBlocks.stoneStalac.blockID, 8, 0x2);
				return true;
			}
		}
		return false;
