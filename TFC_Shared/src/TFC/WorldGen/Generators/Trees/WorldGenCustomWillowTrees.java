...
public class WorldGenCustomWillowTrees extends WorldGenerator
{
		...
		if (random.nextInt (3) == 0)
		{
			...
			for (int a = 0 ; a < length ; a++)
			{
				if(world.getBlockId(X * a / length + x, Y * a / length + y, Z * a / length + z) == 0) {
					world.setBlockAndMetadataWithNotify (X * a / length + x, Y * a / length + y, Z * a / length + z, Block.wood.blockID, treeId, 0);
				}
			}
		...
		for (int y1 = 0 ; y1 < 2 ; y1++)
		{
			for (int x1 = x - 2 + y1 ; x1 < x + 2 - y1 ; x1++)
			{
				for (int z1 = z - 2 + y1 ; z1 < z + 2 - y1 ; z1++)
				{
					...
					{
						world.setBlockAndMetadataWithNotify (x1, y1 + y, z1, Block.leaves.blockID, treeId, 0);
						...
						for (int a = 0 ; a < random.nextInt (2) + 2 ; a++)
						{
							...
							{
								world.setBlockAndMetadataWithNotify (x1, y1 - 1 - a + y, z1, Block.leaves.blockID, treeId, 0);
							}
						}
					}
				}
	...
	private void func_35265_a (World world, int i, int j, int k, int l)
	{
		if(world.getBlockId(i, j, k) == 0 && Block.vine.canBlockStay(world, i, j, k)) {
			world.setBlockAndMetadataWithNotify (i, j, k, Block.vine.blockID, l, 0);
		}
	...
	public boolean generate (World world, Random random, int xCoord, int yCoord, int zCoord)
	{
		...
		if(world.getBlockId(xCoord, yCoord - 1, zCoord) == 0) {
			...
			world.setBlockAndMetadataWithNotify(xCoord, yCoord - 1, zCoord, TFC_Core.getTypeForGrass(rockLayer1.data2), TFC_Core.getSoilMetaFromStone(rockLayer1.data1, rockLayer1.data2), 0);
		}
		int z1, x1, y,x,z;
		y = height+yCoord;
		x = xCoord;
		...
		for (int l1 = 0 ; l1 < height ; l1++)
		{
			...
			{
				world.setBlockAndMetadataWithNotify (xCoord, yCoord + l1, zCoord, Block.wood.blockID, treeId, 0);
			}
		}
