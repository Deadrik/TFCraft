...
public class WorldGenKapokTrees extends WorldGenerator
{
	...
	int checkBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger)
	{
		...
		else
		{
			...
			for (var15 = var3[var5] + var8; var14 != var15; var14 += var8)
			{
				...
				if (var16 != 0 && var16 != 18 && var16!= Block.vine.blockID && var16 != TFCBlocks.Leaves.blockID)
				{
					break;
				}
			}
	...
	void generateLeafNode(int par1, int par2, int par3)
	{
		...
		for (int var5 = par2 + this.leafDistanceLimit; var4 < var5; ++var4)
		{
			...
			this.genTreeLayer(par1, var4, par3, var6, (byte)1, TFCBlocks.Leaves.blockID);
		}
	}
	...
	void generateBranchPoints(){
		...
		for(int j1 = basePos[1]+height-branchRange; j1 < basePos[1]+height;j1++){			
				...
				placeBlockLineCurved(branchOrigin,branchDestination,TFCBlocks.Wood.blockID);			
		}
	}
	...
	void generateLeafNodeBases()
	{
		...
		for (int[] var3 = new int[] {this.basePos[0], this.basePos[1], this.basePos[2]}; var1 < var2; ++var1)
		{
			...
			{
				//this.placeBlockLine(var3, var5, TFCBlocks.LogNatural2.blockID);
			}
		}
	}
	...
	void generateLeaves()
	{
		...
		for (int var2 = this.leafNodes.length; var1 < var2; ++var1)
		{
			...
			//this.placeBlockLine(trunkBottom, node, TFCBlocks.LogNatural2.blockID);
		}
	}
	...
	void generateTrunk()
	{
		...
		this.placeBlockLine(var5, var6, TFCBlocks.Wood.blockID);
		...
		for (var11[par5] = var10[par5]; var12 <= var7; ++var12)
		{
			...
			while (var13 <= var7)
			{
				...
				else
				{
					...
					{
						this.setBlockAndMetadata(this.worldObj, var11[0], var11[1], var11[2], par6, treeId);
						++var13;
					}
				}
			}
	...
	void placeBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, int par3)
	{
		...
		if (var4[var6] != 0)
		{
			...
			for (int var16 = var4[var6] + var9; var15 != var16; var15 += var9)
			{
				...
				if(worldObj.getBlockId(var14[0], var14[1], var14[2]) == 0 || worldObj.getBlockId(var14[0], var14[1], var14[2]) == TFCBlocks.Leaves.blockID ||
						worldObj.getBlockId(var14[0], var14[1], var14[2]) == TFCBlocks.Leaves2.blockID)
				{
					this.setBlockAndMetadata(this.worldObj, var14[0], var14[1], var14[2], par3, treeId);
				}
			}
				...
				for(int i = -1; i <2;i++){
					for(int k = -1; k < 2; k++){
						if(rand.nextBoolean() &&  par1ArrayOfInteger[1] > 1&& Math.abs(i)!=Math.abs(k) && !worldObj.isAirBlock(i + par1ArrayOfInteger[0], par1ArrayOfInteger[1]-1, par1ArrayOfInteger[2]+k)){
							this.setBlockAndMetadata(this.worldObj,i + par1ArrayOfInteger[0], par1ArrayOfInteger[1], par1ArrayOfInteger[2]+k, par3, treeId);
						}
					}
				}
			}
	...
	void placeBlockLinePrimary(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, int par3)
	{
		int[] var4 = new int[] {0, 0, 0};
		byte var5 = 0;
		byte var6;
	...
	void placeBlockLineCurved(int[] startingPos, int[] endingPos,int par3)
	{	
	...
	void placeBlockLineSecondary(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, int par3)
	{
		int[] var4 = new int[] {0, 0, 0};
		byte var5 = 0;
		byte var6;
