...
public class BlockTerraSed extends BlockCollapsable
{
    ...
    {			
        dropBlockAsItem_do(world, i, j, k, new ItemStack(mod_TFC_Core.terraStoneSedCobble, 1, damageDropped(l)));
                ...
                for(int y = -4; y <= 1; y++)
                {
                    for(int x = -range; x <= range; x++)
                    {
                        for(int z = -range; z <= range; z++)
                        {
                            ...
                            {
                                if( Block.blocksList[world.getBlockId(i+x,  j+y, k+z)] instanceof BlockCollapsable && tryToFall(world, i+x, j+y, k+z,world.getBlockMetadata( i+x, j+y, k+z)))
                                {
                                    int done = 0;
                                    while(done < height)
                                    {
