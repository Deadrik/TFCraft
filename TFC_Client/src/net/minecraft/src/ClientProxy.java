...
public class ClientProxy extends CommonProxy 
{
	...
	public int grassColorMultiplier(IBlockAccess par1IBlockAccess, int i, int j, int k)
	{
		...
		for (int z = -1; z <= 1; ++z)
		{
			for (int x = -1; x <= 1; ++x)
			{
				int var10 = TFC_Climate.getGrassColor(getCurrentWorld(), i, j ,k);
				var5 += (var10 & 16711680) >> 16;
			var6 += (var10 & 65280) >> 8;
		var7 += var10 & 255;
			}
	...
	public int foliageColorMultiplier(IBlockAccess par1IBlockAccess, int i, int j, int k)
	{
		...
		if(par1IBlockAccess.getBlockId(i, j, k) == TFCBlocks.fruitTreeLeaves.blockID)
		{
			...
			else
			{
				for (int var8 = -1; var8 <= 1; ++var8)
				{
					for (int var9 = -1; var9 <= 1; ++var9)
					{
						int var10 = TFC_Climate.getFoliageColor(getCurrentWorld(), i, j ,k);
						rgb = applyColor(var10, rgb);
					}
				}
		...
		else if(par1IBlockAccess.getBlockId(i, j, k) == Block.vine.blockID)
		{
					...
					!(((TFCWorldChunkManager)getCurrentWorld().provider.worldChunkMgr).getEVTLayerAt(i, k).floatdata1 < 0.8) && TFC_Climate.getHeightAdjustedTemp(j, k) < 30)
			{
				...
				for (int var8 = -1; var8 <= 1; ++var8)
				{
					for (int var9 = -1; var9 <= 1; ++var9)
					{
						color = (int) (TFC_Climate.getFoliageColor(getCurrentWorld(), i, j ,k) * (1-daymult));
					...
					!(((TFCWorldChunkManager)getCurrentWorld().provider.worldChunkMgr).getEVTLayerAt(i, k).floatdata1 < 0.8) && TFC_Climate.getHeightAdjustedTemp(j, k) < 30)
			{
				...
				for (int var8 = -1; var8 <= 1; ++var8)
				{
					for (int var9 = -1; var9 <= 1; ++var9)
					{
				int color = (int) (TFC_Climate.getFoliageColor(getCurrentWorld(), i, j ,k) * daymult);
				rgb = applyColor(color, rgb);
					}
				}
			...
			else
			{
				for (int var8 = -1; var8 <= 1; ++var8)
				{
					for (int var9 = -1; var9 <= 1; ++var9)
					{
						int color = (int) (TFC_Climate.getFoliageColor(getCurrentWorld(), i, j ,k));
						rgb = applyColor(color, rgb);
					}
				}
		...
		{
			if(TFC_Time.currentMonth >= TFC_Time.September && (meta == 9 || meta == 8 || meta == 12))
			{
				for (int var8 = -1; var8 <= 1; ++var8)
				{
					for (int var9 = -1; var9 <= 1; ++var9)
