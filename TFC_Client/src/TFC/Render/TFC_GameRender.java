...
public class TFC_GameRender 
{
    ...
    public static boolean renderAnvil(Block block, int i, int j, int k, RenderBlocks renderblocks)
    {
        ...
        if(((TileEntityTerraAnvil)blockAccess.getBlockTileEntity(i, j, k)).AnvilTier == AnvilReq.STONE.Tier)
        {
            ...
            int tex = ModLoader.getMinecraftInstance().renderEngine.getTexture("/bioxx/terraRock.png");
            ModLoader.getMinecraftInstance().renderEngine.bindTexture(tex);
            
            renderblocks.overrideBlockTexture = TFC_CoreRender.getRockTexture(j,biome);
