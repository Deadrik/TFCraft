...
public class EntityPolarBear extends EntityTameable implements ICausesDamage, IAnimal, IInnateArmor
{
	...
	protected void applyEntityAttributes()
	{
		...
		this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(TFC_MobData.POLAR_BEAR_HEALTH);//MaxHealth
	}

	@Override
	public boolean attackEntityAsMob (Entity par1Entity)
	{
		int dam = (int) (TFC_MobData.POLAR_BEAR_DAMAGE * getStrengthMod() * getAggressionMod() * (getSizeMod() / 2 + 0.5F));
	...
	public boolean canMateWith (EntityAnimal par1EntityAnimal)
	{
		if (par1EntityAnimal == this)
			return false;
		if (!(par1EntityAnimal instanceof EntityPolarBear))
			return false;
		EntityPolarBear entitybear = (EntityPolarBear) par1EntityAnimal;
	...
	public boolean canMateWith(IAnimal animal) 
	{
		return animal.getGender() != this.getGender() &&this.isAdult() && animal.isAdult() &&
				animal instanceof EntityPolarBear;
	}
	...
	public EntityAgeable createChildTFC(EntityAgeable eAgeable)
	{
		...
		return new EntityPolarBear(worldObj, this, data);
	}
	...
	protected void dropFewItems(boolean par1, int par2)
	{
		...
		float foodWeight = ageMod * (this.sizeMod * 1500);
		...
		if (!worldObj.isRemote)
		{
			...
			if (isPregnant())
			{
				if (TFC_Time.getTotalTicks() >= timeOfConception + pregnancyRequiredTime)
				{
					...
					for (int x = 0; x < i; x++)
					{
						EntityPolarBear baby = (EntityPolarBear) createChildTFC(this);//new EntityBear(worldObj, this,data);
