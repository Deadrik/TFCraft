...
public class ItemCoal extends ItemTerra {
	...
	private boolean createPile(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z, int side, int l)
	{
		...
		if(te != null)
		{
			...
			{
				te.storage[0] = new ItemStack(this,3,itemstack.getItemDamage());
				te.storage[1] = new ItemStack(this,3,itemstack.getItemDamage());
				te.storage[2] = new ItemStack(this,3,itemstack.getItemDamage());
				te.storage[3] = new ItemStack(this,3,itemstack.getItemDamage());
			}
		}
	...
	public boolean isValid(World world, int i, int j, int k) {
		if (world.isSideSolid(i, j - 1, k, ForgeDirection.UP)) {
			...
			if (te instanceof TECoalPile) {
				...
				if (lp.storage[0] == null || lp.storage[0].stackSize < 3)
				{
					return false;
				}
				if (lp.storage[1] == null || lp.storage[1].stackSize < 3)
				{
					return false;
				}
				if (lp.storage[2] == null || lp.storage[2].stackSize < 3)
				{
					return false;
				}
				if (lp.storage[3] == null || lp.storage[3].stackSize < 3)
				{
					return false;
				}
			}
