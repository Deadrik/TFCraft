...
public class BlockWoodSupport2 extends BlockTerra
{
	...
	public static Boolean getSupportInRange(World world, int x, int y, int z, int range, boolean isVerticalSupport)
	{
		if(isVerticalSupport)
		{
			...
			{
				if(TFCBlocks.isBlockVSupport(world.getBlock(x+i, y, z)))
					return true;
				if(TFCBlocks.isBlockVSupport(world.getBlock(x-i, y, z)))
					return true;
				if(TFCBlocks.isBlockVSupport(world.getBlock(x, y, z+i)))
					return true;
				if(TFCBlocks.isBlockVSupport(world.getBlock(x, y, z-i)))
					return true;
			}
		...
		if(!world.isRemote && canPlaceBlockAt(world, x, y, z))
		{
			...
			if(!TFCBlocks.isBlockVSupport(block))
			{
				if(side == 0 && world.isAirBlock(x, y-1, z) /*&& world.getBlock(x, y, z).isOpaqueCube()*/)
				{
					...
					if(isNextToSupport(world,x,y,z-1) != 0 && (getSupportInRange(world, x,y,z-1,5,true) || getSupportInRange(world, x,y-1,z-1,5,true)) || TFCBlocks.isBlockVSupport(world.getBlock(x, y-1, z-1)))
						return true;
					...
					if(isNextToSupport(world,x-1,y,z) != 0  && (getSupportInRange(world, x-1,y,z,5,true) || getSupportInRange(world, x-1,y-1,z,5,true)) || TFCBlocks.isBlockVSupport(world.getBlock(x-1, y-1, z)))
						return true;
