...
public class RenderPipeBasic  implements ISimpleBlockRenderingHandler 
{
			...
			Block block, int modelId, RenderBlocks renderer) 
	{
			...
			boolean isValve = world.getBlockId(i, j, k) == TFCBlocks.SteamPipeValve.blockID;
			//Origin is from top
			if(l == 0){
				...
				if(isValve){
					...
					if(toggle == 0){
						...
						renderer.renderStandardBlock(Block.blockRedstone, i, j, k);
					}
					else{
			...
			int index = -1;
			blocks[0] = Block.blocksList[blockAccess.getBlockId(i, j+1, k)];
			blocks[1] = Block.blocksList[blockAccess.getBlockId(i-1, j, k)];
			blocks[2] = Block.blocksList[blockAccess.getBlockId(i, j, k-1)];
			blocks[3] = Block.blocksList[blockAccess.getBlockId(i, j, k+1)];
			blocks[4] = Block.blocksList[blockAccess.getBlockId(i+1, j, k)];
			blocks[5] = Block.blocksList[blockAccess.getBlockId(i, j-1, k)];
